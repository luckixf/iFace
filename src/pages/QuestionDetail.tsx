		...
		<div style={{ display: "flex", alignItems: "center", gap: 12 }}>
			...
			<div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: "auto" }}>
				<div style={{ width: 80, height: 4, background: "var(--border)", borderRadius: 99, overflow: "hidden" }}>
					<div style={{ height: "100%", background: "var(--primary)", borderRadius: 99, width: `${percent}%`, transition: "width 0.4s var(--ease-out)" }} />
				</div>
				<span style={{ fontSize: 12, fontWeight: 500, color: "var(--text-2)", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap" }}>
					{current} / {total}
				</span>
			</div>
		</div>
...
function ShortcutHints({ answerVisible }: { answerVisible: boolean }) {
	return (
		<div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", fontSize: 12, color: "var(--text-3)" }}>
			{!answerVisible && (
			...
			{answerVisible && (
				<>
					<span style={{ display: "flex", alignItems: "center", gap: 4 }}><Kbd>1</Kbd><span>没掌握</span></span>
					<span style={{ display: "flex", alignItems: "center", gap: 4 }}><Kbd>2</Kbd><span>大概会</span></span>
					<span style={{ display: "flex", alignItems: "center", gap: 4 }}><Kbd>3</Kbd><span>完全掌握</span></span>
				</>
			)}
			<span style={{ display: "flex", alignItems: "center", gap: 4 }}><Kbd>→</Kbd><span>下一题</span></span>
			<span style={{ display: "flex", alignItems: "center", gap: 4 }}><Kbd>←</Kbd><span>上一题</span></span>
		</div>
	);
}
...
function StreakCelebration({ streak, onDone }: StreakCelebrationProps) {
	...
	return (
		...
		>
			<span style={{ fontSize: 64, lineHeight: 1, filter: "drop-shadow(0 4px 12px rgba(0,0,0,0.18))" }}>
				{emoji}
			</span>
			<div
				style={{
...
风格：直接、简练、不废话，总字数控制在200字以内。用中文回答。`;
	...
	return (
		<div style={wrapperStyle}>
			...
			>
				<span style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", display: "flex", alignItems: "center", gap: 6 }}>
					<span style={{
						...
						<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
							<path d="M12 20h9" /><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z" />
						</svg>
					</span>
					我的作答
				...
				<button
					...
					style={{ background: "none", border: "none", color: "var(--text-3)", cursor: "pointer", fontSize: 11, padding: "3px 6px", borderRadius: 5, transition: "all 0.15s" }}
			...
			<div style={{ padding: compact ? "10px 0 0" : "12px 16px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
			...
			{!displayFeedback && (
									...
									<span style={{ fontSize: 11, color: "var(--text-3)" }}>
										{isAiEnabled ? (
											...
											<button
												...
												style={{ background: "none", border: "none", color: "var(--primary)", cursor: "pointer", fontSize: 11, padding: 0, display: "flex", alignItems: "center", gap: 4 }}
											>
				...
				{(displayFeedback || isStreaming) && (
					...
					>
						...
						<div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border-subtle)", background: "var(--surface-3)" }}>
							<p style={{ fontSize: 11, color: "var(--text-3)", marginBottom: 4 }}>我的作答</p>
							<p style={{ fontSize: 12, color: "var(--text-2)", lineHeight: 1.55, whiteSpace: "pre-wrap" }}>{text}</p>
						</div>

						{/* AI feedback */}
						<div style={{ padding: "12px 14px" }}>
							<div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
								<div style={{ width: 18, height: 18, borderRadius: 5, background: "var(--primary-light)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
									<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
										<path d="M12 2a2 2 0 0 1 2 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 0 1 7 7h1a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1h-1v1a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-1H2a1 1 0 0 1-1-1v-3a1 1 0 0 1 1-1h1a7 7 0 0 1 7-7h1V5.73c-.6-.34-1-.99-1-1.73a2 2 0 0 1 2-2z" />
										<circle cx="7.5" cy="14.5" r="1.5" /><circle cx="16.5" cy="14.5" r="1.5" />
									</svg>
								</div>
								<span style={{ fontSize: 12, fontWeight: 500, color: "var(--text-2)" }}>AI 点评</span>
								{isStreaming && (
									<span style={{ display: "inline-flex", gap: 3, marginLeft: 4 }}>
										{[0,1,2].map(i => (
											<span key={i} style={{ width: 4, height: 4, borderRadius: "50%", background: "var(--primary)", display: "inline-block", animation: `ai-dot-bounce 1.2s ease-in-out ${i * 0.2}s infinite` }} />
										))}
									</span>
								)}
							</div>
						...
						{feedback && !isStreaming && (
							<div style={{ padding: "8px 14px", borderTop: "1px solid var(--border-subtle)", display: "flex", gap: 8 }}>
								<button
									onClick={handleReset}
									style={{ fontSize: 12, color: "var(--text-3)", background: "none", border: "none", cursor: "pointer", padding: "3px 6px", borderRadius: 6, transition: "all 0.15s" }}
								...
								<button
									onClick={onOpenAIPanel}
									style={{ fontSize: 12, color: "var(--primary)", background: "none", border: "none", cursor: "pointer", padding: "3px 6px", borderRadius: 6, transition: "all 0.15s", display: "flex", alignItems: "center", gap: 4 }}
				...
				{error && (
					<div style={{ padding: "9px 12px", borderRadius: 9, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", fontSize: 12, color: "#ef4444" }}>
						{error}
						<button onClick={() => setError(null)} style={{ marginLeft: 8, background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontSize: 12, textDecoration: "underline", padding: 0 }}>
							关闭
						</button>
					</div>
				)}
			...
			>
				...
				>
					...
					<div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
						...
						<span style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", flexShrink: 0 }}>AI 助手</span>
						{config.model && (
							<span
								style={{
									fontSize: 10,
...
export default function QuestionDetail() {
	...
	const handleSetStatus = useCallback(
		...
		[id, marking, setStatus, isInSession, nextId, navigate, sessionIds, incrementStreak, streak.currentStreak],
	);
	...
	if (loading) {
		return (
			<div className="page-container animate-fade-in" style={{ maxWidth: 760, display: "flex", flexDirection: "column", gap: 16 }}>
		...
		return (
			<div className="page-container" style={{ maxWidth: 760 }}>
				<div className="card" style={{ padding: "48px 24px", textAlign: "center" }}>
					...
					<Link to="/questions">
						<Button variant="primary" size="sm">返回题库</Button>
					</Link>
				</div>
			</div>
		);
			...
			>
				...
				<div className="animate-fade-in">
					{isInSession ? (
						...
						<nav style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--text-3)" }}>
							<Link
								to="/questions"
							...
							<span style={{ color: "var(--text-2)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 200 }}>
				...
				{/* Question Card */}
				<div className="card animate-fade-in stagger-1" style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
					{/* Meta row */}
					...
					{/* Question text */}
					<h1 style={{ fontSize: 17, fontWeight: 600, color: "var(--text)", lineHeight: 1.65, letterSpacing: "-0.005em" }}>
						{question.question}
					</h1>
					...
					>
						...
						<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: 14, borderBottom: "1px solid var(--border-subtle)" }}>
							<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
								<div style={{ width: 3, height: 18, borderRadius: 99, background: "var(--primary)", flexShrink: 0 }} />
							...
							<button
								...
								style={{
									...
									borderRadius: 7,
									border: "1px solid",
									borderColor: aiDrawerOpen ? "rgba(var(--primary-rgb),0.4)" : "var(--border-subtle)",
				...
				{streak.currentStreak >= 2 && (
					...
					>
						...
						<span style={{ fontWeight: 600, color: "var(--text)", fontVariantNumeric: "tabular-nums" }}>{streak.currentStreak}</span>
						<span>连击</span>
