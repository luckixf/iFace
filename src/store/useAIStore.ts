...
export const DEFAULT_SYSTEM_PROMPT = `你是一位专业的建工考试教练，帮助用户理解公路工程管理与实务、建设工程法规及相关知识、建设工程施工管理三类题目，并梳理答题思路。

## 核心原则
- **简洁优先**：聚焦核心点，避免冗长罗列
- **考试导向**：优先突出考点、作答步骤、易错点和记忆抓手
- **理解优于死记**：不仅给结论，也说明为什么这样答
- **即时可用**：给出用户能立刻套用的答题结构和关键词

## 首次分析格式（仅用于初始问题分析）
1. 先给出 **1 句话核心结论**
2. 再用 **2-4 个要点** 展开（每点不超过 2 行）
3. 如有必要，附上 **1 个公式、流程或记忆口诀**
4. 可指出 **1 个加分延伸点**

## 追问与对话规则
- 追问、闲聊、深挖细节时，**直接自然地回答**，不要套用上方固定格式
- 保持对话连贯性，像真实的教练一样随机应变
- 不替用户机械背答案，而是帮用户理解后再复述
- 只回答与建工考试刷题、考点解析和答题训练相关的问题
    ...
    (hasAnswer: boolean) => [
      {
        ...
        label: '分析考点',
        icon: '🎯',
        prompt: '请分析这道题的核心考点、命题意图，以及答题时最容易漏掉的关键信息。',
      },
      {
        id: 'structure',
        label: '作答框架',
      ...
      },
      {
        ...
        icon: '🔍',
        prompt:
          '请逐项解释本题每个选项为什么正确或错误。请按题目实际给出的选项顺序说明，先给出“正确/错误/不确定”的结论，再用考试考点解释原因，并指出容易混淆的表述。如果当前还没有参考答案，请明确说明这是基于题干和选项的初步判断。',
      },
      ...(hasAnswer
        ? [
            {
              ...
              label: '精炼答案',
              icon: '✨',
              prompt: '请帮我把参考答案整理成更适合考试复述的版本，并标出容易得分的关键词。',
            },
          ]
        : []),
      {
        ...
        label: '记忆抓手',
        icon: '🔮',
        prompt: '请把这道题提炼成 3-5 个最值得记忆的抓手，可以是口诀、步骤或关键词。',
      },
      {
        id: 'pitfalls',
        label: '踩坑提醒',
        icon: '⚠️',
        prompt: '这道题常见的误区和陷阱有哪些？我应该注意避免哪些错误表述？',
      },
      {
        id: 'practice',
        label: '案例追问',
